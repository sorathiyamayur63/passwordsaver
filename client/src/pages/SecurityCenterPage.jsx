...
export const SecurityCenterPage = () => {
  ...
  const runScan = () => {
    if (!items || items.length === 0) {
      setReport(calculateVaultHealthScore([]));
      return;
    }
    setIsScanning(true);
    setReport(null);
  ...
  const TABS = [
    { id: 'weak',     label: 'Weak Passwords',  count: report?.raw?.passStrength?.summary?.weakCount || 0, icon: ShieldAlert },
    { id: 'reused',   label: 'Reused Passwords', count: report?.raw?.reused?.length || 0,                  icon: AlertTriangle },
    { id: 'old',      label: 'Old Passwords',    count: report?.raw?.old?.length || 0,                     icon: Clock },
    { id: 'expiring', label: 'Expiring Items',   count: (report?.raw?.expiring?.expired?.length || 0) + (report?.raw?.expiring?.expiringSoon?.length || 0), icon: AlertCircle },
  ];
    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-fadeIn">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Security Center</h1>
      ...
      {isScanning && (
        <Card padding="lg" className="flex flex-col items-center justify-center py-20 text-center border-[var(--border)]">
          <div className="relative h-16 w-16 mb-4">
            <div className="absolute inset-0 border-4 border-[var(--bg-secondary)] rounded-full" />
          ...
          <p className="text-sm text-[var(--text-muted)] mt-2">Checking entropy and reuse patterns locally.</p>
        </Card>
      )}

      {/* Report — shown after scan */}
      {report && !isScanning && (
        <div className="space-y-6 animate-fadeIn">

          {/* Empty vault message */}
          {report.grade === 'N/A' && (
            <Card padding="lg" className="flex flex-col items-center justify-center py-16 text-center border-[var(--border)]">
              <ShieldCheck className="h-16 w-16 text-[var(--text-muted)] mb-4 opacity-50" />
              <h2 className="text-lg font-semibold text-[var(--text-primary)]">No vault items found</h2>
              <p className="text-sm text-[var(--text-secondary)] max-w-md mt-2">
                Add passwords, cards, or other items to your vault to receive a security health score.
              </p>
            </Card>
          )}

          {/* Full report — only when vault has items */}
          {report.grade !== 'N/A' && (
                ...
                <div className="flex border-b border-[var(--border)] bg-[var(--bg-secondary)] overflow-x-auto">
                  {TABS.map(tab => (
                    ...
                    >
                      ...
                      {tab.label}
                      {tab.count > 0 && (
                        <Badge variant={activeTab === tab.id ? 'info' : 'default'} size="sm" className="ml-2">
                          {tab.count}
                        </Badge>
                      )}
                    </button>
                          ...
                          <div key={i} className="p-4 flex items-center justify-between hover:bg-[var(--bg-tertiary)]">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                ...
                                <div className="h-1 w-16 bg-[var(--danger)] rounded-full" />
                              </div>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              {group.items.map(item => (
                                <Badge
                                  key={item.uuid}
                                  variant="default"
                                  className="cursor-pointer hover:bg-[var(--border)]"
                                  onClick={() => navigate(`/vault?item=${item.uuid}`)}
                                >
                                  {item.title}
                                </Badge>
                              ))}
